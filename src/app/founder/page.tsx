...
export default function FounderConsolePage() {
  ...
  if (sessionStatus === 'loading' || loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0A0A0A] text-white">
        ...
        <span className="text-xs uppercase tracking-widest text-white/40">Authenticating Immortal Access...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0A0A0A] text-white p-6">
        ...
        <p className="text-xs text-white/40 mt-2 max-w-md text-center">{error}</p>
        <button 
          onClick={() => window.location.reload()}
          className="mt-6 px-4 py-2 bg-[#1A1A1A] hover:bg-[#252525] border border-white/10 text-xs uppercase font-bold tracking-widest rounded"
        >
          Retry Connection
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white p-8">
      {/* Header Banner */}
      <div className="border border-white/5 bg-[#121212]/50 p-8 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 relative overflow-hidden backdrop-blur-xl">
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#D4AF37]/5 rounded-full filter blur-[80px] -z-10 pointer-events-none" />
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 bg-[#D4AF37]/15 border border-[#D4AF37]/30 text-[#D4AF37] text-[9px] font-bold uppercase tracking-widest rounded-full">
              👑 Immortal
            </span>
            <span className="text-[10px] text-white/40 uppercase tracking-widest">Founder Console</span>
          </div>
          <h1 className="text-3xl font-light tracking-tight">Supreme Platform Control</h1>
          <p className="text-xs text-white/50">Permanent platform ownership, operational governance & security override.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={fetchSummary}
            className="p-2.5 bg-[#1C1C1C] hover:bg-[#252525] border border-white/5 hover:border-white/10 rounded-lg text-white/60 hover:text-white transition-all"
          ...
          <button
            ...
            className="px-4 py-2 bg-[#1C1C1C] hover:bg-[#252525] border border-white/5 text-xs uppercase tracking-widest font-semibold rounded-lg transition-all"
          >
            Exit Console
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-[#121212] border border-white/5 p-6 rounded-xl relative overflow-hidden group">
          <div className="absolute top-4 right-4 text-white/5 group-hover:text-[#D4AF37]/10 transition-colors">
            <Shield size={20} />
          </div>
          <span className="text-[10px] text-white/40 uppercase tracking-widest font-semibold block">System Integrity</span>
          ...
          <span className="text-[10px] text-white/40 block mt-2 border-t border-white/5 pt-2">Governance Lock Enforced</span>
        </div>
        ...
        <div className="bg-[#121212] border border-white/5 p-6 rounded-xl relative overflow-hidden group">
          ...
          <span className="text-[10px] text-white/40 uppercase tracking-widest font-semibold block">Primary Super Admins</span>
          <span className="text-2xl font-light text-[#D4AF37] mt-2 block">{data?.stats?.totalPrimarySAs} Active</span>
          <span className="text-[10px] text-white/40 block mt-2 border-t border-white/5 pt-2">Operational SAs registered</span>
        </div>

        <div className="bg-[#121212] border border-white/5 p-6 rounded-xl relative overflow-hidden group">
          <div className="absolute top-4 right-4 text-white/5 group-hover:text-red-500/10 transition-colors">
            <ShieldAlert size={20} />
          </div>
          <span className="text-[10px] text-white/40 uppercase tracking-widest font-semibold block">Security Threat Alerts</span>
          ...
          <span className="text-[10px] text-white/40 block mt-2 border-t border-white/5 pt-2">SOC Threat Registry feed</span>
        </div>
        ...
        <div className="bg-[#121212] border border-white/5 p-6 rounded-xl relative overflow-hidden group">
          ...
          <span className="text-[10px] text-white/40 uppercase tracking-widest font-semibold block">Platform Mode</span>
          <span className={`text-2xl font-light mt-2 block ${data?.settings?.global_lockdown ? 'text-red-500 animate-pulse' : 'text-[#D4AF37]'}`}>
            {data?.settings?.global_lockdown ? '🔒 LOCKDOWN' : '🟢 ACTIVE'}
          </span>
          <span className="text-[10px] text-white/40 block mt-2 border-t border-white/5 pt-2">Global system mutability</span>
        </div>
      </div>

      {/* Tabs Layout */}
      <div className="flex border-b border-white/5 gap-2 mb-8 overflow-x-auto">
        {[
      ...
      <div className="space-y-8">
        {activeTab === 'governance' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 bg-[#121212] border border-white/5 p-6 rounded-xl space-y-6">
              <h2 className="text-lg font-light tracking-wide border-b border-white/5 pb-3 flex items-center gap-2">
              ...
              <div className="space-y-4">
                {data?.primarySAs?.length === 0 ? (
                  <div className="p-8 text-center text-white/30 italic text-xs border border-white/5 rounded-lg">
                    No Primary Super Administrators registered.
                  </div>
                ) : (
                  data?.primarySAs?.map((sa: any) => (
                    <div key={sa.id} className="p-4 bg-[#0A0A0A] border border-white/5 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-[#D4AF37]/30 transition-all">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-white">{sa.name || 'Anonymous Admin'}</span>
                        ...
                        <p className="text-[11px] text-white/40 mt-1">{sa.email}</p>
              ...
              <div className="bg-[#0A0A0A] border border-white/5 p-6 rounded-xl space-y-4">
                <h3 className="text-xs uppercase tracking-widest text-[#D4AF37] font-bold">Appoint Primary Super Admin</h3>
                <form onSubmit={handlePromote} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[9px] uppercase tracking-widest text-white/40 font-bold block">Select Administrator</label>
                      <select
                        value={selectedAdminId}
                        onChange={(e) => setSelectedAdminId(e.target.value)}
                        required
                        className="w-full bg-[#161616] border border-white/5 rounded px-3 py-2 text-xs text-white/80 focus:border-[#D4AF37]/45 focus:outline-none"
                      >
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[9px] uppercase tracking-widest text-white/40 font-bold block">Promotion Justification Reason</label>
                      <input
                        type="text"
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#D4AF37] hover:bg-[#C5A030] text-black text-[10px] uppercase tracking-widest font-bold rounded transition-colors disabled:opacity-50"
                  >
                    Promote user
                  </button>
                </form>
            ...
            {/* Governance Timeline */}
            <div className="lg:col-span-4 bg-[#121212] border border-white/5 p-6 rounded-xl space-y-4">
              <h2 className="text-sm font-semibold uppercase tracking-wider text-white/80 border-b border-white/5 pb-2">
                Governance Audit Log
              </h2>
              {data?.governanceHistory?.length === 0 ? (
                <div className="p-8 text-center text-white/30 italic text-xs">No records available.</div>
              ) : (
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                  {data?.governanceHistory?.map((log: any) => (
                    <div key={log.id} className="text-xs border-b border-white/5 pb-3 last:border-0 last:pb-0 space-y-1.5">
                      <div className="flex justify-between items-start">
                        <span className="font-semibold text-[#D4AF37]">{log.reason || 'Governance Action'}</span>
                        <span className="text-[9px] text-white/40">{new Date(log.createdAt).toLocaleDateString()}</span>
                      </div>
                      <p className="text-[10px] text-white/60">
                        Actor ID: <code className="text-white/40 bg-white/5 px-1 rounded">{log.actorId}</code>
                      </p>
                      <div className="flex gap-2 text-[9px]">
                        <span className="text-white/40">Previous: {log.previousRole || 'NONE'}</span>
        ...
        {activeTab === 'soc' && (
          <div className="bg-[#121212] border border-white/5 p-6 rounded-xl space-y-6">
            <h2 className="text-lg font-light tracking-wide border-b border-white/5 pb-3 flex items-center gap-2">
                ...
                {data?.securityAlerts?.map((alert: any) => (
                  <div key={alert.id} className="p-4 bg-[#110505] border border-red-500/25 rounded-lg space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[10px] text-white/40">{new Date(alert.createdAt).toLocaleString()}</span>
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-xs font-semibold text-red-200">{alert.description}</h4>
                      <p className="text-[10px] text-white/40">Type: {alert.type}</p>
                    </div>
                    <div className="text-[10px] text-white/50 border-t border-red-500/10 pt-2 flex justify-between">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-[#121212] border border-white/5 p-6 rounded-xl space-y-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-center p-3 bg-[#0A0A0A] border border-white/5 rounded-lg">
            ...
            <div className="bg-[#121212] border border-white/5 p-6 rounded-xl space-y-6">
              ...
              <div className="space-y-4 text-xs">
                <div className="p-4 bg-[#0A0A0A] border border-white/5 rounded-lg space-y-2">
                  <div className="flex justify-between">
                    <span className="font-semibold text-white">Founder Email</span>
                    <code className="text-white/40">{data?.systemIntegrity?.founderStatus?.email}</code>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-semibold text-white">Active Primary Super Admins</span>
                    <span className="text-[#D4AF37]">{data?.systemIntegrity?.primarySACount} active</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-semibold text-white">Integrity validation status</span>
                    <span className="text-green-400">PASSED</span>
                  </div>
                </div>

                <div className="p-4 bg-[#D4AF37]/5 border border-[#D4AF37]/25 rounded-lg text-white/70 space-y-2">
                  <h4 className="font-bold text-[#D4AF37] text-xs flex items-center gap-1">
                    <Info size={14} />
                ...
                <button
                  ...
                  className={`w-full py-2.5 rounded text-[10px] uppercase tracking-widest font-extrabold border transition-all ${
                    data?.settings?.global_lockdown
                      ...
                      : 'bg-red-500 border-red-500 text-black hover:bg-red-600 hover:border-red-600'
                  }`}
                >
              ...
              <div className="p-6 bg-[#0E1524] border border-blue-500/20 rounded-xl space-y-4 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <Eye size={16} className={data?.settings?.read_only ? 'text-blue-500' : 'text-white/40'} />
                  </div>
                  <p className="text-[10px] text-white/60 leading-relaxed">
              ...
              <div className="p-6 bg-[#1C160B] border border-amber-500/20 rounded-xl space-y-4 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <Radio size={16} className={data?.settings?.maintenance_mode ? 'text-amber-500 animate-pulse' : 'text-white/40'} />
                  </div>
                  <p className="text-[10px] text-white/60 leading-relaxed">
