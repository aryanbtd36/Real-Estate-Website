...
export default function SuperAdminDashboard() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 text-slate-800 p-6 space-y-6 animate-pulse">
        <div className="h-8 w-48 bg-slate-50 rounded" />
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map(i => <div key={i} className="h-24 bg-slate-50 rounded-xl border border-slate-200/80" />)}
        </div>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 space-y-8 pb-16">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-200/80 pb-6">
        <div>
          <h1 className="text-3xl font-light tracking-wide flex items-center gap-2">
            <Compass className="text-[#0B4C8C] animate-pulse" size={28} />
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="py-2 px-4 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded text-[10px] uppercase font-bold tracking-wider text-[#0B4C8C] hover:text-[#0B4C8C] transition-colors"
          >
            Launch Security SOC
          </Link>
          <button
            ...
            disabled={refreshing}
            className="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 rounded text-slate-650 hover:text-slate-800 transition-colors"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className="bg-white border border-slate-200/80 p-4 rounded-xl space-y-1 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-b from-[#0B4C8C]/5 to-transparent rounded-full blur-lg" />
            <span className="text-[8px] uppercase tracking-widest text-slate-500 font-bold block">{c.label}</span>
            <div className="text-2xl font-semibold text-[#0B4C8C] tracking-tight truncate">{c.val}</div>
            <span className="text-[9px] text-slate-400 block mt-0.5">{c.desc}</span>
          </div>
        ))}
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200/80 p-6 rounded-2xl space-y-4">
            <h3 className="text-xs uppercase tracking-widest font-semibold text-slate-650 flex items-center gap-1.5">
              <UserCheck size={13} className="text-[#0B4C8C]" />
              Staff Admin Trust & Risk Rankings
            </h3>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-200/80 text-slate-500 uppercase tracking-widest font-mono text-[9px] pb-3">
                ...
                <tbody className="divide-y divide-slate-200">
                  {stats.adminRiskRankings?.map((adm: any) => (
                    <tr key={adm.id} className="hover:bg-white/[0.01]">
                      <td className="py-3 font-semibold text-slate-800">{adm.name}</td>
                      <td className="py-3 font-mono text-[10px] text-slate-650">{adm.role}</td>
          ...
          <div className="bg-white border border-slate-200/80 p-6 rounded-2xl space-y-4">
            ...
            <div className="space-y-3">
              {alerts.length === 0 ? (
                <div className="text-center py-8 border border-dashed border-slate-200/80 rounded-xl text-xs text-slate-500">
                  Zero active threats recorded.
                </div>
              ) : (
                alerts.slice(0, 5).map((a: any) => (
                  <div key={a.id} className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl flex justify-between items-center gap-3">
                    <div>
                      <span className="text-xs font-semibold text-slate-800 block">{a.description}</span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200/80 p-6 rounded-2xl space-y-4">
            ...
            <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
              {geoEvents.length === 0 ? (
                <div className="text-center py-8 text-xs text-slate-400">Zero geo anomalies flagged.</div>
              ) : (
                geoEvents.map((e: any) => (
                  <div key={e.id} className="p-2.5 bg-slate-50 border border-slate-200/80 rounded-lg space-y-1">
          ...
          <div className="bg-white border border-slate-200/80 p-6 rounded-2xl space-y-4">
            ...
            <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
              {behaviorEvents.length === 0 ? (
                <div className="text-center py-8 text-xs text-slate-400">No behavioral anomalies found.</div>
              ) : (
