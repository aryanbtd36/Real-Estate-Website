...
function ApartmentsCatalogContent() {
  ...
  const fallbackApartments: Property[] = [
    {
      id: 'apt-1',
      name: 'Faizabad Road Premium 3 BHK Flat',
    ...
    {
      id: 'apt-2',
      name: 'Gomti Nagar Heights Condominium',
      ...
      try {
        ...
        if (res.ok) {
          ...
          const list = (Array.isArray(data) ? data : []).filter(
            (p: any) =>
              (p.type.toLowerCase() === 'apartment' || p.type.toLowerCase() === 'flat' || p.type.toLowerCase() === 'condo') &&
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased pb-16">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-24 space-y-8">
        ...
        <div className="border-b border-slate-200/60 pb-6 text-left">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight mt-1">Verified Flats & Society Apartments</h1>
          <p className="text-sm text-slate-500 mt-1.5 font-medium">Browse boundary-surveyed flats and premium apartments validated by state registry office audits.</p>
        </div>
        ...
        <div className="sticky top-[68px] z-30 bg-white/95 backdrop-blur-md border border-slate-200/60 p-4 rounded-[20px] shadow-premium flex flex-wrap gap-4 items-center">
          <div className="relative flex-1 min-w-[200px]">
            <input
              type="text"
              placeholder="Search apartments by neighborhood or keyword..."
              value={searchQuery}
              onChange={(e) => {
                  ...
                  {paginated.map((apt) => (
                    ...
                    >
                      <div className="relative h-44 bg-slate-100">
                        {(() => {
                          const coverImg = apt.imagesRelation?.find(img => img.isCover)?.url || 
                                           apt.imagesRelation?.[0]?.url || 
                                           (apt.images ? apt.images.split(',')[0] : 'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=600&auto=format&fit=crop&q=80');
                          return (
                            <img
                              src={coverImg}
                              alt={apt.name}
                              className="w-full h-full object-cover"
                              onError={(e) => handleImageError(e, apt.type)}
                            />
                          );
                        })()}
                        ...
                        <div className="absolute bottom-3 right-3 bg-slate-950/70 text-white text-[10px] px-2.5 py-1 rounded font-bold backdrop-blur-xs">
                          {apt.area} {apt.areaUnit || 'Sq Ft'}
                        </div>
                      </div>
                      <div className="p-5 flex-1 flex flex-col justify-between">
                        <div>
                          <span className="text-[9px] uppercase font-black tracking-widest text-slate-400">{apt.bedrooms > 0 ? `${apt.bedrooms} BHK` : ''} Apartment</span>
                          <h3 className="font-bold text-slate-900 text-sm mt-1.5 line-clamp-1">{apt.name}</h3>
            ...
            <div className="h-[360px] md:h-[420px] rounded-xl overflow-hidden border border-slate-200/60">
              {selectedProp ? (
                ...
                <div className="w-full h-full bg-slate-100 rounded-lg flex items-center justify-center text-slate-400">
                  Select an apartment listing to survey coordinates
                </div>
              )}
            </div>
