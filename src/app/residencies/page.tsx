...
export default function ResidenciesCatalogPage() {
  ...
  const fallbackResidencies: Property[] = [
    ...
    {
      id: 'res-2',
      name: 'Gomti Nagar Modern Duplex',
      location: 'Sector 5, Gomti Nagar, Lucknow',
      price: 14500000,
      bedrooms: 4,
      area: 3200,
      areaUnit: 'Sq Ft',
    ...
    {
      id: 'res-3',
      name: 'Faizabad Road Independent House',
      ...
      try {
        ...
        if (res.ok) {
          ...
          const list = (Array.isArray(data) ? data : []).filter(
            (p: any) =>
              (p.type.toLowerCase() === 'villa' || p.type.toLowerCase() === 'duplex' || p.type.toLowerCase() === 'independent house' || p.type.toLowerCase() === 'residency') &&
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased pb-16">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-24 space-y-8">
        ...
        <div className="border-b border-slate-200/60 pb-6 text-left">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight mt-1">Verified Duplexes & Independent Villas</h1>
          <p className="text-sm text-slate-500 mt-1.5 font-medium">Browse boundary-surveyed residencies and luxury duplexes validated by state registry office audits.</p>
        </div>
        ...
        <div className="sticky top-[68px] z-30 bg-white/95 backdrop-blur-md border border-slate-200/60 p-4 rounded-[20px] shadow-premium flex flex-wrap gap-4 items-center">
          <div className="relative flex-1 min-w-[200px]">
            <input
              type="text"
              placeholder="Search residencies by neighborhood or keyword..."
              value={searchQuery}
              onChange={(e) => {
          ...
          <div className="flex flex-wrap gap-3 items-center w-full lg:w-auto">
            ...
            >
              ...
              <option value="3600">3600+ Sq Ft</option>
            </select>
                    ...
                    >
                      <div className="relative h-44 bg-slate-100">
                        ...
                        <div className="absolute bottom-3 right-3 bg-slate-950/70 text-white text-[10px] px-2.5 py-1 rounded font-bold backdrop-blur-xs">
                          {res.area} {res.areaUnit || 'Sq Ft'}
                        </div>
                      </div>
                      <div className="p-5 flex-1 flex flex-col justify-between">
                        <div>
                          <span className="text-[9px] uppercase font-black tracking-widest text-slate-400">{res.bedrooms > 0 ? `${res.bedrooms} BHK` : ''} Residency</span>
                          <h3 className="font-bold text-slate-900 text-sm mt-1.5 line-clamp-1">{res.name}</h3>
            ...
            <div className="h-[360px] md:h-[420px] rounded-xl overflow-hidden border border-slate-200/60">
              {selectedProp ? (
                ...
                <div className="w-full h-full bg-slate-100 rounded-lg flex items-center justify-center text-slate-400">
                  Select a residency listing to survey coordinates
                </div>
              )}
            </div>
