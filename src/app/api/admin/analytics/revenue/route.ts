...
import { RevenueAnalyticsService } from '@/lib/analytics/revenue';
...
export async function GET() {
  try {
    ...
    const cacheKey = 'analytics_revenue';
    let data = AnalyticsCache.get<any>(cacheKey);

    if (!data) {
      data = await RevenueAnalyticsService.getRevenueAnalytics();
    ...
    await ActivityService.log({
      ...
      description: 'Report generated: Revenue Pipeline Value and Trends',
      details: { report: 'revenue_analytics' },
    });

    return NextResponse.json(data);
  } catch (error) {
    console.error('[API.admin.analytics.revenue] GET error:', error);
