...
export function AdminSidebar() {
  ...
  return (
    <aside className="w-full md:w-72 bg-[#0F172A] border-r border-slate-800 flex flex-col justify-between p-6 shrink-0 md:min-h-screen">
      <div className="space-y-8">
        {/* Logo */}
        <Link href="/" className="flex items-center space-x-2">
          <span className="text-2xl font-black tracking-[0.2em] text-white">AURA</span>
          <span className="text-[10px] tracking-[0.4em] uppercase text-slate-400 border-l border-slate-700 pl-2">ESTATE</span>
        </Link>
 
        {/* User Badge */}
        <div className="p-4 bg-slate-900 rounded-xl border border-slate-800 flex flex-col gap-2">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-full bg-[#0B4C8C]/20 border border-[#0B4C8C]/30 flex items-center justify-center text-white font-semibold">
              {session?.user?.name?.charAt(0) || 'A'}
            </div>
            <div className="overflow-hidden flex-1">
              <span className="text-[10px] text-slate-400 block font-bold truncate uppercase">{displayTitle}</span>
          ...
          <div className="flex justify-start">
            <span className={`text-[8px] font-extrabold tracking-wider px-2 py-0.5 rounded border ${
              (session?.user as any)?.isFounder ? 'bg-[#0B4C8C]/15 border-[#0B4C8C]/30 text-white' :
            ...
            return (
              <Link
                ...
                className={`flex items-center space-x-3 px-4 py-3 rounded text-sm text-left transition-colors ${
                  active
                    ? 'bg-[#0B4C8C]/20 text-white border-l-2 border-[#0B4C8C]'
      ...
      <div className="space-y-2 pt-6 border-t border-slate-800 mt-6">
        <Link
          href="/"
          className="flex items-center space-x-3 px-4 py-3 rounded text-sm text-slate-400 hover:text-white hover:bg-white/5 transition-colors"
        >
          <HomeIcon size={18} />
        ...
        <button
          ...
          className="w-full flex items-center space-x-3 px-4 py-3 rounded text-sm text-red-400 hover:bg-red-500/10 transition-colors text-left"
        >
          <LogOut size={18} />
          <span>Sign Out</span>
        </button>
