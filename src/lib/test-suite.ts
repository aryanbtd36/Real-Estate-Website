...
async function runTestSuite() {
  ...
  try {
    ...
    const checkRoleModificationSafety = (actor: any, target: any, newRole: string) => {
      if (actor.role === UserRole.ADMIN && (target.role === UserRole.PRIMARY_SUPER_ADMIN || target.role === UserRole.FOUNDER_SUPER_ADMIN)) {
    ...
    assert(founderUser?.role === RoleType.FOUNDER_SUPER_ADMIN, 'Founder: Role is FOUNDER_SUPER_ADMIN', 'Role must match');
    ...
    assert(founderUser?.role === RoleType.FOUNDER_SUPER_ADMIN, 'Auto-repair: Tampered role restored to FOUNDER_SUPER_ADMIN', 'Restoration of role failed');

    // Verify logs generated by repair
    let repairGovLog = await db.governanceHistory.findFirst({
      where: { targetUserId: founderUser!.id, actorId: 'SYSTEM_BOOTSTRAP', newRole: RoleType.FOUNDER_SUPER_ADMIN },
      orderBy: { createdAt: 'desc' }
    });
    assert(repairGovLog !== null, 'Repair Logs: GovernanceHistory recorded for role repair', 'No history log found');
    assert(repairGovLog?.newRole === RoleType.FOUNDER_SUPER_ADMIN, 'Repair Logs: Role history value is correct', 'New role mismatch');
    assert(repairGovLog?.targetUserId === founderUser!.id, 'Repair Logs: GovernanceHistory target is Founder user');
    assert(repairGovLog?.previousRole === RoleType.FOUNDER_SUPER_ADMIN, 'Repair Logs: GovernanceHistory previousRole is FOUNDER_SUPER_ADMIN');
    ...
    const runBlockTest = async (actionDesc: string, actorId: string, testPrefix: string) => {
      ...
      assert(gov!.newRole === RoleType.FOUNDER_SUPER_ADMIN, `${testPrefix} Logs: New role in log remains unchanged`, 'Role mismatch');
      assert(gov!.targetUserId === founderUser!.id, `${testPrefix} Logs: GovernanceHistory target matches Founder`);
      assert(gov!.previousRole === RoleType.FOUNDER_SUPER_ADMIN, `${testPrefix} Logs: GovernanceHistory previous role is FOUNDER_SUPER_ADMIN`);
    };
    ...
    assert(primaryUser?.role === RoleType.PRIMARY_SUPER_ADMIN, 'Primary SA: Role is PRIMARY_SUPER_ADMIN', 'Role mismatch');
    ...
    assert(primaryUser?.role === RoleType.PRIMARY_SUPER_ADMIN, 'Primary SA: Tampered role restored to PRIMARY_SUPER_ADMIN', 'Restoration failed');
    ...
    const simulatePromotion = async (actor: any, target: any, action: 'PROMOTE' | 'DEMOTE') => {
      if (actor.role !== 'FOUNDER_SUPER_ADMIN') {
        throw new Error('Forbidden: Only the Founder can assign Super Admin roles.');
      }
      return db.user.update({
        where: { id: target.id },
        data: {
          role: action === 'PROMOTE' ? RoleType.PRIMARY_SUPER_ADMIN : RoleType.ADMIN,
    ...
    assert(promoSuccess.role === RoleType.PRIMARY_SUPER_ADMIN && promoSuccess.isPrimarySA === true, 'Primary SA Promotion: Founder successfully promoted Admin to Primary SA', 'Promotion failed');
    ...
    const simulateDemotion = async (actor: any, target: any) => {
      ...
      const psaCount = await db.user.count({
        where: { role: RoleType.PRIMARY_SUPER_ADMIN, deletedAt: null }
      });
      if (psaCount <= 1) {
    ...
    assert(lockDeactGovLog!.newRole === RoleType.FOUNDER_SUPER_ADMIN, 'Lockdown Mode: Deactivation newRole is Founder');
